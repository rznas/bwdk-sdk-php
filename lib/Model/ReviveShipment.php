...
/**
 * ReviveShipment Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class ReviveShipment implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'ReviveShipment';
     ...
     *
     * @param int|null $preparation_time The preparation time for the order in days.
     *
     * @return self
     */
    public function setPreparationTime($preparation_time)
    {
        if (is_null($preparation_time)) {
            array_push($this->openAPINullablesSetToNull, 'preparation_time');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('preparation_time', $nullablesSetToNull);
            if ($index !== FALSE) {
        ...
        if (!is_null($preparation_time) && ($preparation_time < 0)) {
            throw new \InvalidArgumentException('invalid value for $preparation_time when calling ReviveShipment., must be bigger than or equal to 0.');
        }
