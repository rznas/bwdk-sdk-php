...
class Merchant implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setName($name)
    {
        ...
        if (!is_null($name) && (mb_strlen($name) > 128)) {
            throw new \InvalidArgumentException('invalid length for $name when calling Merchant., must be smaller than or equal to 128.');
        }
    ...
    public function setDomain($domain)
    {
        ...
        if ((mb_strlen($domain) > 200)) {
            throw new \InvalidArgumentException('invalid length for $domain when calling Merchant., must be smaller than or equal to 200.');
        }
